...
public class CountInstancesWithTimeSlice extends AbstractLocalSyncWorker {
    ...
    protected void onWork(Object request, Object response) throws WorkerException {
        if (request instanceof RequestEntity) {
            ...
            RangeQueryBuilder registryTimeWithinQueryTime = QueryBuilders.rangeQuery(InstanceIndex.REGISTRY_TIME).gte(requestEntity.getStartTime()).lte(requestEntity.getEndTime());
            RangeQueryBuilder pingTimeWithinQueryTime = QueryBuilders.rangeQuery(PingTimeIndex.PING_TIME).gte(requestEntity.getStartTime()).lte(requestEntity.getEndTime());
